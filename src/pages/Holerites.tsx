...
export default function Holerites() {
  const holerites = useEntity<any>("holerites");
                  ...
                  <TableRow key={item.id} className="hover:bg-muted/30">
                    <TableCell>
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-medium text-foreground">{MESES[(item.mes ?? 1) - 1]} / {item.ano}</p>
                          <p className="text-sm text-muted-foreground">{item.tipo === "13" ? "13º Salário" : item.tipo === "ferias" ? "Férias" : "Mensal"}{item.valor_liquido ? ` • R$ ${Number(item.valor_liquido).toFixed(2)}` : ""}</p>
                        </div>
                      </div>
                    </TableCell>
